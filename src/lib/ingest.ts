...
function buildGenerationPrompt(schema: string, purpose: string, index: string, sourceFileName: string, overview?: string, wikiDirs?: string[]): string {
  ...
  return [
    ...
    `2. Entity/concept/strategy/stock pages in the appropriate wiki subdirectory. Available directories: ${wikiDirs && wikiDirs.length > 0 ? wikiDirs.join(", ") : "wiki/entities/, wiki/concepts/"}. Follow the Wiki Schema above to decide which directory matches each page type (e.g., stocks → wiki/股票/, strategies → wiki/策略/, patterns → wiki/模式/, general entities → wiki/entities/, general concepts → wiki/concepts/).`,
