...
function buildGenerationPrompt(schema: string, purpose: string, index: string, sourceFileName: string, overview?: string, wikiDirs?: string[]): string {
  ...
  return [
    ...
    `   CRITICAL RULES: (a) You MUST use ONLY the directories listed above. Do NOT create any new directories. (b) If a Chinese directory exists for a page type (e.g. wiki/股票/, wiki/策略/, wiki/模式/), you MUST use the Chinese directory and NEVER use its English equivalent (e.g. wiki/stocks/, wiki/strategies/, wiki/patterns/). (c) Follow the Wiki Schema to decide which directory matches each page type.`,
